...
def main(args):
    ...
    if args.curriculum_type == "data":
        pacing_scheduler = PacingScheduler(
            method=args.pacing,
            alpha0=args.starting_percent,
            step=args.step,
            lambda_=args.linear_growth,
            r=args.exponential_growth,
        )
        train_loader, val_loader, curriculum_dataset = get_data_curriculum_dataloaders(
            args.dataset,
            args.batch_size,
            pacing_scheduler,
            args.model_name,
            args.balance_batches,
            val_indices_path=args.val_indices_path,
        )
        update_callback = UpdateDataLoaderCallback(
            curriculum_dataset, args.batch_size, shuffle=True
        )
        callbacks_list = [
            performance_checkpoint_callback,
            periodic_checkpoint_callback,
            update_callback,
        ]
        model = timm.create_model(
            args.model_name, num_classes=args.num_classes, pretrained=args.pretrained
        )

    elif args.curriculum_type == "task":
        train_loader, val_loader = get_c2f_dataloaders(
            args.dataset,
            args.batch_size,
            args.task_curr_stage,
            args.model_name,
            args.balance_batches,
            val_indices_path=args.val_indices_path,
        )
        ...
        if args.task_curr_stage == 1:
            ...
            if args.prev_checkpoint and os.path.exists(args.prev_checkpoint):
                print(
                    f"Loading checkpoint from {args.prev_checkpoint} for task curriculum stage 2..."
                )
                ...
                for k, v in state_dict.items():
                    ...
                    new_state[new_key] = v
                model = timm.create_model(
                    args.model_name, num_classes=args.old_classes, pretrained=False
                )
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        print(
            f"Batch {batch_idx}: data shape {data.shape}, target shape {np.array(target).shape}"
        )
        output = model(data)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="data",
        choices=["data", "task"],
        help="Type of curriculum experiment",
    )
    parser.add_argument(
        "--dataset", type=str, default="CIFAR100", help="Dataset to use"
    )  # e.g., CIFAR100 or imagenet
    parser.add_argument("--batch_size", type=int, default=64, help="Batch size")

    # Model and Training Settings
    parser.add_argument(
        "--model_name", type=str, default="resnet18", help="Model architecture to use"
    )
    parser.add_argument(
        "--num_classes", type=int, default=100, help="Number of output classes"
    )
    ...
    parser.add_argument(
        "--balance_batches",
        action="store_true",
        help="Balance classes per batch if set",
    )
    parser.add_argument(
        "--pretrained",
        action="store_true",
        help="Use pretrained model on imagenet in Data curriculum or coarse task curriculum",
    )
    parser.add_argument(
        "--old_classes", type=int, default=20, help="Number of old output classes"
    )
    parser.add_argument(
        "--optimizer",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of epochs for which the pacing function remains constant",
    )
    parser.add_argument(
        "-p",
        "--starting_percent",
        type=float,
        default=0.3,
        help="Fraction of data used in the initial step",
    )
    parser.add_argument(
        "-l",
        "--linear_growth",
        type=float,
        default=0.1,
        help="Linear growth factor to expand the data used in each step",
    )
    parser.add_argument(
        "-r",
        "--exponential_growth",
        type=float,
        default=1.5,
        help="Exponential growth factor to expand the data used in each step",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="staged",
        choices=["staged", "continuous"],
        help="Variant used for coarse-to-fine curriculum learning",
    )
    parser.add_argument(
        "--task_curr_stage",
        type=int,
        default=1,
        choices=[1, 2],
        help="Curriculum stage: coarse (1) or fine (2)",
    )
    ...
    parser.add_argument(
        ...
        help="NPZ file with pre‑computed train/val indices",
    )
