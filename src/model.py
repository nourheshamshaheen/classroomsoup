...
def continuous_loss(logits, cluster_targets, coarse_mapping):
    ...
    for i in range(logits.size(0)):
        ...
        if len(indices) == 0:
            raise ValueError(
                f"No fine classes found for cluster {cluster} in the coarse mapping."
            )
...
class LitClassifier(pl.LightningModule):
    def __init__(
        ...
        self.curriculum_dataset = (
            curriculum_dataset  # may contain additional info for curriculum training.
        )
        # For the continuous variant, we require the coarse_mapping.
        if self.variant == "continuous" and coarse_mapping is None:
            raise ValueError(
                "For continuous variant, you must provide a coarse_mapping."
            )
    ...
    def configure_optimizers(self):
        optimizer_map = {
            ...
            "sgd": lambda params, **kwargs: torch.optim.SGD(
                params, momentum=0.9, **kwargs
            ),
