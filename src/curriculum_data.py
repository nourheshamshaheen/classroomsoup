...
class CoarseLabelDataset(Dataset):
    ...
    def __init__(self, base_dataset, mapping_fn):
        ...
        try:
            ...
            self.targets = [self.mapping_fn(base_dataset[i][1]) for i in range(len(base_dataset))]
...
def stratified_split(dataset, difficulties, val_ratio=0.2, random_state=42, save_to=None, load_from=None):
    """
...
class CurriculumDataset(Dataset):
    ...
    def update_indices(self):
        ...
        num_samples = int(fraction * len(self.base_dataset))
        self.selected_indices = self.sorted_indices[:num_samples]
        print(f"Updated curriculum dataset: using {num_samples} samples out of {len(self.base_dataset)}.")
...
def get_data_curriculum_dataloaders(dataset_name, batch_size, pacing_scheduler, model=None, balance_batches=False, val_indices_path=None):
    ...
    if dataset_name == "CIFAR100":
        ...
        train_dataset, train_difficulties, val_dataset = stratified_split(dataset, difficulties, val_ratio=0.2, random_state=42, load_from=val_indices_path)
    elif dataset_name.lower() == "imagenet":
        # For ImageNet, load the official train and validation splits.
        train_dataset = datasets.ImageFolder(root='/datashare/ImageNet/ILSVRC2012/train', transform=transform)
        val_dataset   = datasets.ImageFolder(root='/datashare/ImageNet/ILSVRC2012/val', transform=transform)
        difficulties = get_c_scores(dataset_name)['scores']  # Assumes difficulties correspond to the train set.
        # We use the full train_dataset for curriculum learning.
        train_difficulties = difficulties  
    else:
        raise NotImplementedError(f"Dataset {dataset_name} not implemented.")
    
    curriculum_dataset = CurriculumDataset(train_dataset, train_difficulties, pacing_scheduler)
    
    train_loader = get_a_train_dataloader(curriculum_dataset, batch_size, balance_batches)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)
    return train_loader, val_loader, curriculum_dataset

def get_c2f_dataloaders(dataset_name, batch_size, task_stage, model=None, balance_batches=False, val_indices_path=None):
    """
    ...
    if dataset_name == "CIFAR100":
        ...
        original_dataset = datasets.CIFAR100(root='./data', train=True, download=True, transform=transform)
        if task_stage == 1:
            # Get the hard-coded mapping.
            cifar_fine_to_coarse = get_cifar_mapping()  # This returns a dict mapping fine -> coarse.
            cifar_mapping_fn = lambda fine_label: cifar_fine_to_coarse[fine_label]
            full_dataset = CoarseLabelDataset(original_dataset, mapping_fn=cifar_mapping_fn)
        else:
        ...
        val_dataset   = datasets.ImageFolder(root='/datashare/ImageNet/ILSVRC2012/val', transform=transform)
        if task_stage == 1:
            imagenet_fine_to_coarse = get_imagenet_mapping()  # mapping: fine label index -> coarse label (string)
            imagenet_mapping_fn = lambda fine_label: imagenet_fine_to_coarse[fine_label]
            train_dataset = CoarseLabelDataset(train_dataset, mapping_fn=imagenet_mapping_fn)
            val_dataset = CoarseLabelDataset(val_dataset, mapping_fn=imagenet_mapping_fn)
    else:
        raise NotImplementedError(f"Dataset {dataset_name} not implemented.")
...
def get_testing_dataloader(dataset_name, batch_size, model=None):
    ...
    if dataset_name_l == "cifar100":
        ...
        test_dataset = datasets.ImageFolder(
            root="/datashare/ImageNet/ILSVRC2012/val",
            transform=transform
        )
    else:
