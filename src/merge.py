...
def greedy_soup(
    model_paths, model_names, model_scores, validate_fn, val_loader, device, model_ctor
):
    ranked = sorted(
        zip(model_names, model_paths), key=lambda x: model_scores[x[0]], reverse=True
    )
...
def iterative_greedy_soup(
    ...
    for ep in range(1, epochs + 1):
        ...
        for name, path in zip(names[1:], paths[1:]):
            ...
            print(
                f"Trying {name:25s}  solo={model_scores[name]:.4f}  merged={new_f1:.4f}"
            )
            if new_f1 > current_f1:
...
def fisher_weighted_averaging(model_dicts, fisher_mats, lam=None, eps: float = 1e-8):
    ...
    lam = torch.as_tensor(
        lam or [1.0] * len(model_dicts), dtype=torch.float32, device=device
    )
...
if __name__ == "__main__":
    ...
    # core
    P.add_argument(
        "--technique", choices=["greedy", "iterative", "fisher"], required=True
    )
    ...
    if args.technique in ["greedy", "iterative"]:
        if args.validate_each_checkpoint:
            ...
            for n in names:
                ...
                if m:
                    model_scores[n] = float(m.group(1))
    ...
    if args.technique == "fisher":
        ...
        if args.fisher_paths:  # pre‑computed
            fisher_mats = [
                torch.load(fp, map_location="cpu") for fp in args.fisher_paths
            ]
        ...
        merged = greedy_soup(
            paths, names, model_scores, validate_fn, val_loader, device, model_ctor
        )
    else:
        merged = iterative_greedy_soup(
            paths,
            names,
            model_scores,
            validate_fn,
