...
def load_checkpoint_into_model(checkpoint_path, model_name, num_classes, device):
    ...
    stripped = {
        re.sub(r"^(?:model\.|pl_module\.)+", "", k): v for k, v in state.items()
    }
...
def main(args):
    ...
    model = load_checkpoint_into_model(
        args.checkpoint, args.model_name, args.num_classes, device
    )

    test_loader = get_testing_dataloader(
        args.dataset, args.batch_size, model=args.model_name
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate a checkpoint on the test set"
    )
    parser.add_argument(
        "--checkpoint", required=True, help="Path to .ckpt or .pth file"
    )
    parser.add_argument(
        "--model_name", default="resnet18", help="timm model architecture"
    )
    parser.add_argument(
        "--num_classes",
        type=int,
    ...
    parser.add_argument(
        "--dataset", default="CIFAR100", help="CIFAR100 or ImageNet (case‑insensitive)"
    )
    parser.add_argument(
        "--batch_size", type=int, default=128, help="Batch size for the test loader"
    )
