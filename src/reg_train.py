...
def get_regular_dataloaders(dataset_name, batch_size, val_ratio=0.2, model_name=None, val_indices_path=None):
    """
...
def train_model(model, train_loader, val_loader, logger, checkpoint_callbacks, max_epochs, lr):
    """
...
def main(args):
    ...
    train_loader, val_loader = get_regular_dataloaders(args.dataset, args.batch_size, model_name=args.model_name, val_indices_path=args.val_indices_path)
    
    # Create the model from timm. Use pretrained weights if available.
    model = timm.create_model(args.model_name, num_classes=args.num_classes, pretrained=True)
    
    print("Starting training with regular (non-curriculum) experiment...")
    train_model(model, train_loader, val_loader, logger, [checkpoint_callback], args.max_epochs, args.lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_comet", action="store_true",
                        help="Enable Comet logging if set")
    parser.add_argument("--val_indices_path", type=str, default="split/cifar100_val_split.npz",
