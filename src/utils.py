...
class UpdateDataLoaderCallback(Callback):
    ...
    def on_train_epoch_start(self, trainer, pl_module):
        ...
        pl_module.data_fraction = self.curriculum_dataset.pacing_scheduler.current_fraction
...
class BalancedBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        pools = {label: indices.copy() for label, indices in self.label_to_indices.items()}
        
        all_batches = []
        while True:
            batch = []
            # Try to sample uniformly across labels
            labels = random.sample(self.labels, k=min(len(self.labels), self.batch_size))
            for label in labels:
                if pools[label]:
                    idx = pools[label].pop()
                    batch.append(idx)
